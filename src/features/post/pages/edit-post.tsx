...
import { X, ArrowLeft, Save, Eye, Upload, Loader2, Image as ImageIcon, Sparkles, FileText } from "lucide-react";
import useAuthStore from "@/features/auth/hooks/use-auth-store";
import { useUpdatePost, useGetCategories, useGetPost } from "../api/use-posts";
...
export function EditPost() {
  ...
  // Existing images from backend (for display only)
  ...
  useEffect(() => {
    if (post) {
      setFormData({
        name: post.name,
        category_id: categories.find((c: any) => c.slug === post.category)?.id || "",
        description: post.description,
      });
      setBannerPreview(post.bannerImage);
      setExistingImages(post.additionalImages || []);
    }
  }, [post, categories]);
  ...
  const handleAdditionalImagesChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files.length > 0) {
      // Limit total new additional images to max 4
      if (additionalFiles.length + files.length > 4) {
        toast.error("Maksimal hanya 4 gambar tambahan baru");
        return;
      }
  ...
  if (isPostLoading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-slate-50 gap-3">
        ...
        <span className="text-sm font-medium text-slate-600">Memuat data postingan...</span>
      </div>
    );
  }
      ...
      <div className="sticky top-0 z-40 w-full bg-white/70 backdrop-blur-md border-b border-slate-200/60 shadow-sm transition-all duration-300">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex items-center justify-between">
          ...
          <Button 
            ...
            className="rounded-xl px-5 py-2.5 bg-gradient-to-r from-indigo-600 to-blue-600 hover:from-indigo-700 hover:to-blue-700 text-white font-semibold flex items-center gap-2 shadow-lg shadow-indigo-500/10 transition-all hover:scale-[1.01] active:scale-[0.99] disabled:opacity-75"
          >
          ...
          <div className="lg:col-span-7 space-y-6">
            ...
            <Card className="border-slate-200/60 shadow-sm bg-white/90 backdrop-blur-sm rounded-2xl overflow-hidden">
              <CardHeader className="border-b border-slate-100 pb-4">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <CardDescription>Sesuaikan informasi postingan Anda</CardDescription>
                  </div>
                </div>
              </CardHeader>
              ...
              <CardContent className="pt-6 space-y-6">
                ...
                {/* Banner Upload */}
                <div className="space-y-2">
                  <Label className="text-slate-700 font-semibold text-sm">Gambar Banner Utama</Label>
                  ...
                  {bannerPreview ? (
                    ...
                    >
                      ...
                      <p className="text-sm font-semibold text-slate-700 group-hover:text-indigo-600 transition-colors">Klik untuk mengganti gambar banner</p>
                ...
                {/* Additional Images */}
                <div className="space-y-2">
                  <Label className="text-slate-700 font-semibold text-sm">Gambar Pendukung Tambahan Baru <span className="text-slate-400 font-normal">(Maksimal 4)</span></Label>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                    {additionalPreviews.map((preview, index) => (
                      <div key={index} className="relative aspect-square rounded-xl overflow-hidden border border-slate-100 group shadow-sm bg-slate-50">
                        <img 
                          src={preview} 
                          alt={`Preview pendukung baru ${index + 1}`} 
                          className="w-full h-full object-cover"
                        />
                        <button
                          type="button"
                    ...
                    {additionalPreviews.length < 4 && (
                      ...
                      >
                        ...
                        <span className="text-xs font-medium text-slate-500 group-hover:text-indigo-600">Tambah Baru</span>
                      </div>
                    )}
                  </div>
            ...
            <Card className="border-slate-200/60 shadow-sm bg-white/90 backdrop-blur-sm rounded-2xl overflow-hidden">
              <CardHeader className="border-b border-slate-100 pb-4">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <CardDescription>Sunting teks deskripsi dengan Rich Text Editor</CardDescription>
                  </div>
                </div>
              </CardHeader>
              ...
              <CardContent className="pt-6">
                <RichTextEditor
                  ...
                  placeholder="Tulis informasi detail di sini secara rinci..."
                />
              </CardContent>
            </Card>
          </div>
            ...
            <div className="backdrop-blur-md bg-white/80 border border-slate-100/80 shadow-xl rounded-3xl overflow-hidden transition-all duration-300">
              ...
              <div className="p-6 space-y-4">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[11px] text-indigo-500 font-semibold bg-indigo-50 border border-indigo-100 px-2 py-0.5 rounded-full flex items-center gap-1">
                    Sedang Diedit
                  </span>
                </div>
                ...
                {(additionalPreviews.length > 0 || existingImages.length > 0) && (
                  <div className="pt-4 border-t border-slate-100">
                    <h4 className="text-xs font-semibold text-slate-500 mb-2">
                      Semua Gambar Pendukung ({additionalPreviews.length + existingImages.length})
                    </h4>
                    <div className="grid grid-cols-2 gap-2">
                      {existingImages.map((img, index) => (
                        <div
                          key={`existing-${index}`}
                          className="aspect-video bg-slate-50 rounded-xl overflow-hidden border border-slate-100 shadow-sm relative group"
                        >
                          <img
                            src={img}
                            ...
                            className="w-full h-full object-cover"
                          />
                          <span className="absolute bottom-1.5 left-1.5 bg-slate-900/60 text-white text-[9px] px-1.5 py-0.5 rounded font-semibold backdrop-blur-sm">
                            GAMBAR LAMA
                          </span>
                        </div>
                      ))}
                      {additionalPreviews.map((img, index) => (
                        <div
                          key={`new-${index}`}
                          className="aspect-video bg-slate-50 rounded-xl overflow-hidden border border-slate-100 shadow-sm relative"
                        >
                          <img
                            src={img}
                            alt={`New ${index + 1}`}
                            className="w-full h-full object-cover"
                          />
                          <span className="absolute bottom-1.5 left-1.5 bg-indigo-600 text-white text-[9px] px-1.5 py-0.5 rounded font-semibold">
                            BARU
                          </span>
                        </div>
                      ))}
